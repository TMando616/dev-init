...
- [x] 6.3 動作確認チェックリスト（ブラウザでの目視確認）
  - [x] レッスン一覧・カテゴリ別一覧で完了済みレッスンに完了マークが表示される（US-1）
  - [x] 未完了のレッスンには完了マークが表示されない（US-1）
  - [x] レッスン詳細画面で「次のレッスンへ」ボタンが表示され、完了済みの場合のみ活性化する（US-2）
  - [x] 未完了の状態では「次のレッスンへ」ボタンが非活性で押下できない（US-2）
  - [x] 最後のレッスンでは「次のレッスンへ」ボタンが表示されない（US-2）
  - [x] 次のレッスンに遷移した際、前のレッスンの実行結果・模範解答表示が残っていない（US-2）
  - [x] サイドバーにカテゴリ一覧が表示され、クリックで該当カテゴリ一覧に遷移する（US-3）
  - [x] 現在表示中のカテゴリがサイドバーでハイライトされる（US-3）
  - [x] サイドバー折りたたみ時もカテゴリ項目が既存ナビと同様に表示される（US-3）
  - [x] 既存の管理画面（レッスン一覧・学習資料関連）に回帰がない（US-1・US-2で変更したAPIレスポンスの影響確認）
