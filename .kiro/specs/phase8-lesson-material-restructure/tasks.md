...
- [ ] 1.1 `materials` を `lesson_id` ベースに再構成するマイグレーションを作成する（design §1-1）
...
- [ ] 3.2 `LessonRepository::find()` / `all()` の eager load を調整する（design §2-3）
...
- [ ] 3.3 `CategoryRepository` の `all()` / `find()` を変更する（design §2-4）
...
- [ ] 4.1 `MaterialService` を `lesson_id` ベースに変更する（design §2-5）
...
- [ ] 5.1 `MaterialController` のバリデーションを `lesson_id` 必須に変更する（US-3、design §2-6）
...
- [ ] 5.2 `LessonController` のカテゴリ選択を必須化する（US-2、design §2-7）
...
- [ ] 6.1 `MaterialSeeder` を `lesson_id` ベースに変更する（design §6）
...
- [ ] 7.5 カテゴリ一覧にレッスン数列とインライン展開を追加する（US-4、design §3-5）
...
- [ ] 8.1 演習ページを `lesson.materials` 直接参照に変更する（US-5、design §4-1）
...
- [ ] 8.2 カテゴリ詳細から materials セクションを削除する（US-6、design §4-2）
...
- [ ] 9.1 `migrate:fresh --seed` が成功することを確認（`docker compose exec backend php artisan migrate:fresh --seed`）
- [ ] 9.2 バックエンドテスト全パス（`docker compose exec backend php artisan test`）— material 関連テストの `category_id` 依存を修正
- [ ] 9.3 フロントエンド lint パス（`docker compose exec frontend npm run lint`）
- [ ] 9.4 動作確認チェックリスト
