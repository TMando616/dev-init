...
- [x] 管理画面から作成したレッスンが、受講生側で実行できる（`/execute` が 422 にならない）
- [x] 管理画面から作成したレッスンが、レッスン一覧で言語サムネイル付きで表示される
...
**完了**（2026-08-02）: 全受け入れ条件を満たし、手動確認も完了。実装時に判明した設計との差分は`design.md` §7 を参照。
